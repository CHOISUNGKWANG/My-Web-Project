...
>
  ...
  <body>
		 ...
		 <form action="search.do" method="post" name="searchForm">
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <h4 class="fw-bold py-3 mb-4"><a href="main.do">LOL Community☆</a></h4>
              ...
              <!-- EL -->
                ...
                <div class="col-md-6 col-lg-4 mb-3" style="width: 100%;">
                  <div class="card h-100">
                    <div class="card-body">
                      <h5 class="card-title" style="float: left;">${bdata.btitle}</h5>
                      <c:if test="${bdata.mid == mid}">
                      <div style="float: right;">
                      	<a href="updatePage.do?bid=${bdata.bid}">수정</a>
                      	<span>|</span>
                      	<a href="deleteBoard.do?bid=${bdata.bid}" class="deleteBoard">삭제</a>
                      </div>
                      </c:if>
                      <div style="float: left; clear: both;">
                      <h6 class="card-subtitle">${bdata.mnick}</h6>
                      <h6 class="text-muted">${bdata.bdate} 조회 ${bdata.bhits}</h6>
                      </div>
                      <div style="float: right;">
                      <c:choose>
                      	<c:when test="${ldata == null}">
                      		<img id="likebtn" alt="좋아요" src="../assets/img/custom/like.png" style="cursor: pointer;">
                      	</c:when>
                      	<c:otherwise>
                      		<img id="likebtn" alt="좋아요" src="../assets/img/custom/like.png" style="background: #696cff; border-radius: 50%; cursor: pointer;">
                      	</c:otherwise>
                      </c:choose>
                      <span id="likecnt">좋아요 ${bdata.blike}</span>
                      </div>
                      <c:if test="${bdata.bimg != null}">
                      <img
                        class="img-fluid d-flex my-4"
                        src="${pageContext.request.contextPath}/file/${bdata.bimg}"
                        alt="이미지"
                        style="clear: both;"
                      />
                      </c:if>
                      <p style="clear: both;" class="card-text">${bdata.bcontent}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <!-- Examples -->
				...
				<div class="card mb-4">
					...
					<!-- EL -->
					<c:forEach var="rs" items="${rdatas}">
					<c:set var="vo" value="${rs.replyVO}" />
					<div class="card-body" style="padding-bottom: 0px; padding-top: 0px;">
                    <hr>
                    <h6 class="card-title" style="float: left;">${vo.mnick}</h6>
                    <c:if test="${vo.mid == mid}">
                    <a href="deleteReply.do?rid=${vo.rid}&bid=${bdata.bid}" class="deleteReply"><img alt="삭제" src="../assets/img/custom/delete.png" style="float:right;"></a>
                    </c:if>
                      <p class="card-text" style="clear:both;">
                        ${vo.rcontent}
                      </p>
                      <h6 class="text-muted">${vo.rdate}&nbsp;&nbsp;&nbsp;&nbsp;
                      <c:if test="${mid != null}">
                      <span class="rreply" id="${vo.rid}" style="cursor: pointer;">답글쓰기</span>
                      </c:if>
                      </h6>
                      
                      <div class="card-body ${vo.rid}" style="display: none; padding-left: 100px;">
                      <div class="card-text" style="border: #696cff solid 2px; padding: 5px;">
                      <form action="insertReply2.do" method="post">
                      	<input type="hidden" name="bid" value="${bdata.bid}">
                        <input type="hidden" name="rid" value="${vo.rid}">
                      	<input type="hidden" name="mid" value="${mid}">
                      	<input type="hidden" name="mnick" value="${mnick}">
                        <p style="margin-bottom: 0;">${mnick}</p>
                        <textarea name="rrcontent" rows="3" style="width: 100%;" placeholder="댓글을 남겨보세요." required="required"></textarea>
                        <div style="text-align: right;">
                        <button class="btn btn-outline-primary" type="submit" style="text-align: right;">댓글 등록</button>
                        </div>
                       </form>
                      </div>
                    </div>
                    ...
                    <c:forEach var="list" items="${rs.reply2List}">
                    <div class="card-body" style="padding-left: 100px; padding-top: 0; padding-bottom: 0px;">
                    <hr>
                    <h6 class="card-title" style="float: left;">${list.mnick}</h6>
                    <c:if test="${list.mid == mid}">
                    <a href="deleteReply2.do?rrid=${list.rrid}&bid=${bdata.bid}" class="deleteReply2"><img alt="삭제" src="../assets/img/custom/delete.png" style="float:right;"></a>
                    </c:if>
                      <p class="card-text" style="clear: both;">
                        ${list.rrcontent}
                      </p>
                      <h6 class="text-muted">${list.rrdate}</h6>
                    </div>
                    </c:forEach>
					</c:forEach>
                    <!-- /EL -->
                    ...
                    <c:if test="${mid != null}">
                     <div class="card-body">
                      ...
                      <form action="insertReply.do" method="post">
                      	...
                      	<input type="hidden" name="mnick" value="${mnick}">
                        <p style="margin-bottom: 0;">${mnick}</p>
                        <textarea name="rcontent" rows="3" style="width: 100%;" placeholder="댓글을 남겨보세요." required="required"></textarea>
                        <div style="text-align: right;">
                        <button class="btn btn-outline-primary" type="submit" style="text-align: right;">댓글 등록</button>
                        </div>
                        </form>
    ...
    <script type="text/javascript">
    	$(".rreply").on("click", function() {
    		var rrid = $(this).attr("id");
    		var target = $("."+rrid);
	...
	<script type="text/javascript">
    	$(".deleteReply, .deleteReply2").on("click", function() {
